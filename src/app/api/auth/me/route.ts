...
const AUTH_ME_RATE_LIMIT_PER_MINUTE = getEnvInt("AUTH_ME_RATE_LIMIT_PER_MINUTE", 120);

export async function GET(req: Request) {
  const requestContext = getRequestContext(req, "/api/auth/me");
  ...
  try {
    ...
    const rate = checkRateLimit({
      key: `rate:auth:me:${clientKey}`,
      limit: AUTH_ME_RATE_LIMIT_PER_MINUTE,
      windowMs: 60_000,
    });
    if (!rate.allowed) {
