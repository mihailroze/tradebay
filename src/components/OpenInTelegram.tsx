...
function buildDeepLink(listingId: string): string {
  const raw = (process.env.NEXT_PUBLIC_TELEGRAM_BOT_USERNAME || "").trim();
  const username = raw.startsWith("@") ? raw.slice(1) : raw;
  if (!username) return "";
  const payload = `l_${listingId}`;
  return `tg://resolve?domain=${encodeURIComponent(username)}&startapp=${encodeURIComponent(payload)}`;
}
...
export default function OpenInTelegram({ listingId }: Props) {
  ...
  useEffect(() => {
    ...
    setDeepLink(buildDeepLink(listingId));
  }, [listingId]);
